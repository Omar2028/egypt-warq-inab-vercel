...
import { createCustomerReviewImage, upsertSiteImage } from "./db";
...
export function registerAdminUploadRoutes(app: Express) {
  ...
  app.post("/api/admin/reviews/upload", async (req, res) => {
    const user = await getGoogleAdminFromRequest(req);
    if (!user) return res.status(403).json({ error: "Forbidden" });
    const parsed = uploadSchema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: "ملف أو بيانات صورة غير صالحة." });
    ...
    if (!data.length || data.length > MAX_IMAGE_BYTES) return res.status(400).json({ error: "حجم الصورة يجب ألا يتجاوز 5 ميغابايت." });
    try {
      const { key, url } = await storagePut(`customer-reviews/${randomUUID()}.${extensions[parsed.data.mimeType]}`, data, parsed.data.mimeType);
      await createCustomerReviewImage({ labelAr: parsed.data.labelAr, altAr: parsed.data.altAr ?? null, storageKey: key, url, mimeType: parsed.data.mimeType, sizeBytes: data.length, updatedBy: user.id });
      return res.status(201).json({ url, key });
    } catch { return res.status(500).json({ error: "تعذر حفظ صورة الرأي. حاول مرة أخرى." }); }
  });
}
