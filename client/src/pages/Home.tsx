...
export default function Home() {
  ...
  const cartItems = useMemo(() => displayMenuItems.filter((item) => (quantities[item.id] ?? 0) > 0).map((item) => ({ ...item, quantity: quantities[item.id], subtotal: item.price * quantities[item.id] })), [displayMenuItems, quantities]);
  ...
  return <main className="site-shell paper-grain" dir="rtl">
    ...
    <section id="top" className="hero-section" aria-labelledby="hero-title"><div className="hero-copy"><div className="eyebrow"><Sparkles size={15} /> {copy("home.eyebrow", "ورق عنب منزلي في القاهرة")}</div><h1 id="hero-title">{heroMain}<span>{heroAccent}</span></h1><p>{heroDescription}</p><div className="hero-actions"><button className="primary-cta" onClick={() => scrollToSection("order")}>{copy("order.cta", "اطلب من المنيو")} <ArrowLeft size={18} /></button><span className="hero-note"><Leaf size={16} /> {copy("home.note", "طازج عند الطلب")}</span></div></div><div className="hero-visual" aria-label="صينية ورق عنب طازج"><div className="hero-image-wrap"><img src={heroPhoto} alt={copy("home.heroAlt", "صينية ورق عنب محضرة طازجة بالليمون")} /></div><div className="hero-sticker"><span>01</span><small>{copy("home.sticker", "اختيار اليوم")}</small></div><div className="hero-side-note">{copy("home.sideNote", "طعم بيتي بشكل جديد")}</div></div></section>
            ...
            <section id="order" className="order-section root-direct-section" aria-labelledby="order-title"><div className="order-intro"><div><div className="section-index">01 <span>/</span> المنيو</div><h2 id="order-title">اطلبها <em>بسهولة.</em></h2></div><p>نوع واحد في البداية، وبعدها كل الأصناف والعدد في قائمة واحدة.</p></div><div className="root-direct-shell"><div className="root-menu-flow"><section className="root-type-step"><div className="root-step-heading"><span>1</span><div><small>أولًا</small><h3>حدد نوع ورق العنب</h3></div></div><div className="root-type-options" role="radiogroup" aria-label="نوع ورق العنب">{orderTypes.map((item) => <button key={item} role="radio" aria-checked={orderType === item} className={orderType === item ? "selected" : ""} onClick={() => setOrderType(item)}>{orderType === item && <Check size={14} />}{item}</button>)}</div></section><section className="root-items-step"><div className="root-step-heading"><span>2</span><div><small>اختار والعدد جنبك</small><h3>كل الأصناف في منيو واحد</h3></div></div><div className="root-menu-list">{displayMenuItems.map((item) => { const qty = quantities[item.id] ?? 0; return <article key={item.id} className={`root-menu-row ${qty > 0 ? "selected" : ""}`}><div className="root-row-copy"><div><small>{item.category}</small>{item.tag && <b>{item.tag}</b>}</div><strong>{item.title}</strong><span>{item.description}{item.category === "ورق عنب" && <em> · {orderType}</em>}</span></div><div className="root-row-price"><strong>{item.price}</strong><small>ج.م</small></div><div className="root-row-quantity"><button aria-label={`إنقاص ${item.title}`} onClick={() => changeQuantity(item.id, -1)} disabled={qty === 0}><Minus size={15} /></button><output aria-label={`عدد ${item.title}`}>{qty}</output><button aria-label={`زيادة ${item.title}`} onClick={() => changeQuantity(item.id, 1)}><Plus size={15} /></button></div></article>})}</div><p className="root-menu-note"><Clock size={15} /> زِد العدد بجانب أي صنف، ثم راجع الإجمالي على اليسار.</p></section></div><aside className="root-order-summary" aria-label="ملخص وإجمالي الطلب"><div className="compact-summary-head"><span>ملخص طلبك</span><ShoppingBag size={17} /></div><div className="summary-type-row"><span>نوع ورق العنب</span><strong>{orderType}</strong></div>{cartItems.length ? <div className="root-summary-items">{cartItems.map((item) => <div key={item.id}><span>{item.title} <small>× {item.quantity}</small></span><strong>{item.subtotal} ج.م</strong></div>)}</div> : <p className="root-summary-empty">زِد العدد بجانب الصنف المطلوب.</p>}<div className="compact-total"><span>الإجمالي النهائي</span><strong>{total} <small>ج.م</small></strong></div><button className="compact-submit" onClick={sendOrder}><Send size={16} /> إرسال الطلب</button><p>سيظهر النوع والكميات في رسالة واتساب.</p></aside></div></section>

    <section id="delivery" className="delivery-strip" aria-labelledby="delivery-title"><div className="delivery-mark"><img src={brandMark} alt="" /></div><div><p>{copy("delivery.label", "التوصيل")}</p><h2 id="delivery-title">{copy("delivery.title", "من مطبخنا")} <em>{copy("delivery.accent", "لبيتك.")}</em></h2></div><div className="delivery-points"><span><MapPin size={18} /> {copy("delivery.areas", "القاهرة — المناطق تضاف قبل الإطلاق")}</span><span><Clock size={18} /> {copy("delivery.prepTime", "تجهيز مسبق حسب الطلب")}</span></div><button className="light-cta" onClick={sendOrder}>{copy("delivery.cta", "اطلب الآن")} <ArrowLeft size={17} /></button></section>
